...
In order to do this, the effect is split in multiple passes. Stating from the light pass, 
4 passes perform erosion and dilation effects. Then, the rest of the watercolor effect is applied 
at the same time as the underwater effects, in the underwater pass.

\medskip \par
\noindent
Finally, the underwater effect (further described in section~\ref{sec:underwater}) is applied at 
the end of the pipeline, in the underwater pass. The underwater effect is therfore computed in 
...
The first modification needed for a watercolor effect is to lower the amount of details in the image by 
