...
\medskip \par
\noindent
Representing those three pigment density variations in OpenGL is quite straightforward when skipping the 
fluid simulation. Instead, each variation is produced per fragment on the fragment shader, using a gray-level
texture. The turbulent flow (low frequencies) is simulated with a Perlin noise texture, the pigment variation 
(high frequencies) is simulated with a Gaussian noise texture, and the paper grain is a simple paper texture.

\begin{figure}[h]
	\centering
	\subfloat[Low frequency texture.]{\includegraphics[width=0.47\columnwidth]{imgs/perlin_noise.jpg}\label{fig:perlin_variation}}
	\hspace{1em}
	\subfloat[High frequency texture.]{\includegraphics[width=0.47\columnwidth]{imgs/gaussian_noise.jpg}\label{fig:pigment_variation}} \\
	\subfloat[Paper grain texture.]{\includegraphics[width=0.47\columnwidth]{imgs/cotton_paper_2.jpg}\label{fig:paper_grain}}
	\hspace{1em}
...
\medskip \par
\noindent
Figure \ref{fig:radial_darkening} shows the results of radial darkening on a scene. Notice the corners of the image. The effect is minimal, and can be hard to notice if it is not pointed out, but adds to the overall image attractiveness.
